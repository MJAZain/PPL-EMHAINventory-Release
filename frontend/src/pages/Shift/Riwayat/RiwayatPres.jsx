...
function RiwayatPresPage() {
  ...
  const fetchShifts = async (page = 1) => {
    ...
    try {
      ...
      const response = await apiClient.get(
        `/sales/prescriptions?limit=${pageSize}&offset=${offset}`
      );

      const formattedData = (response.data.data || []).map(sales => ({
        ...sales,
        prescription_date: formatDateTime(sales.prescription_date),
        doctor_name: sales.doctor?.full_name || "-",
        patient_name: sales.patient?.full_name || "-",
      }));
  ...
  const columns = [ 
    { header: "Diagnosis Pasien", accessor: "diagnosis" },
    { header: "Total Pembelian", accessor: "total_amount" },
    { header: "Tanggal Pembelian", accessor: "prescription_date" },
      ...
      <div className="p-5 w-full py-10">
        ...
        <div className="border-1 rounded-md border-gray-300 bg-white p-5">
          <div className="max-w-[1121px]">
            <DataTable columns={columns} data={shifts} showIndex={false} />
          </div>
...
export default RiwayatPresPage;
