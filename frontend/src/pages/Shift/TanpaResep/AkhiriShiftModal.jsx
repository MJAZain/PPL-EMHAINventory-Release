...
export default function AkhiriShiftModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (isOpen) {
      const balance = Number(localStorage.getItem("closing_balance") || "0");
      const shift = localStorage.getItem("shiftregularId");
...
const handleConfirm = async () => {
  ...
  const totalSales = Number(localStorage.getItem("regular_sale") || "0");
  ...
  try {
    ...
    localStorage.removeItem("shiftregularId");
    localStorage.removeItem("closing_balance");
    localStorage.removeItem("regular_sale");

    onClose();
    navigate("/shift-tanpa-resep");
  } catch (err) {
    console.error(err);
