...
const handleNext = async () => {
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
       ...
       const shiftregularId = response.data?.data?.id;

      if (shiftregularId) {
        localStorage.setItem("shiftregularId", shiftregularId);
      } else {
        console.warn("Shift ID not found in response!");
      }
      
      console.log("shiftregularId:", shiftregularId);
      localStorage.setItem('closing_balance', form.opening_balance);
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="bg-white max-w-xl mx-auto w-full p-6 mt-10 border rounded-md border-gray-300 shadow-md max-h-[90vh] overflow-y-auto">
        <h1 className="text-2xl font-bold text-center mb-6">Buka Shift Kasir Non-Resep</h1>
