...
export default function TanpaResepProductModal({ isOpen, onClose, onSave, initialData = null }) {
  ...
  const handleSelectProduct = (product) => {
    const existingItems = JSON.parse(localStorage.getItem("regularList") || "[]");
  ...
  const handleSave = () => {
    if (!selectedProduct || !form.qty || !form.unit_price) {
    ...
    const newItem = {
      ...
      sub_total: Number(form.qty) * Number(form.unit_price),
    };

    const existing = JSON.parse(localStorage.getItem("regularList") || "[]");

    // Additional check before saving (in case user bypassed the selection check)
    ...
    localStorage.setItem("regularList", JSON.stringify(updated));
    onSave(updated);
    onClose();
  };
              ...
              value={
                form.qty && form.unit_price
                  ? new Intl.NumberFormat("id-ID", {
                      ...
                      .format(Number(form.qty) * Number(form.unit_price))
                      .replace("Rp", "Rp.")
                  : ""
              }
              disabled
